  ...
  Future<void> syncSms(String childUid) async {
    try {
      ...
      final List<dynamic> raw =
          await _ch.invokeMethod('readSms', {'limit': 100});
      final Map<String, dynamic> data = {};
      for (final m in raw) {
        final mm = Map<String, dynamic>.from(m as Map);
        final key =
            '${mm["date"]}_${(mm["address"] as String).replaceAll(RegExp(r"[^0-9+]"), "")}';
        data[key] = mm;
      }
  ...
  static Stream<List<SmsEntry>> watchMessages(String childUid) {
    return FirebaseDatabase.instance
      ...
      final list = map.values
          .map((v) =>
              SmsEntry.fromMap(Map<String, dynamic>.from(v as Map)))
          .toList();
