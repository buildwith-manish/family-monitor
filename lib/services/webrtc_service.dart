...
class WebRTCService {
  ...
  StreamSubscription? _offerSub, _answerSub, _candidateSub, _statusSub, _commandSub;
  ...
  Future<void> startAsChild(String childUid, VoidCallback onCallEnded) async {
    ...
    _candidateSub = _db.child('calls/$childUid/parentCandidates')
        .onChildAdded.listen((e) async {
  ...
  Future<void> startScreenShareAsChild(
    ...
    _offerSub = _db.child('calls/$childUid/offer').onValue.listen((e) async {
      ...
      try {
        ...
        await _db.child('calls/$childUid/answer')
            .set({'sdp': ans.sdp, 'type': ans.type});
      } catch (ex) { debugPrint('[WebRTC] screen answer error: $ex'); }
    });
    _candidateSub = _db.child('calls/$childUid/parentCandidates')
        .onChildAdded.listen((e) async {
      if (e.snapshot.value == null) return;
  ...
  Future<void> startAsParent(String childUid, StreamMode mode,
    ...
    _candidateSub = _db.child('calls/$childUid/childCandidates')
        .onChildAdded.listen((e) async {
      if (e.snapshot.value == null) return;
      try {
        final c = Map<String, dynamic>.from(e.snapshot.value as Map);
