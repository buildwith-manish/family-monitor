...
class _ScheduleLockScreenState extends State<ScheduleLockScreen> {
  ...
  void initState() {
    super.initState();
    _lockSub = _svc.watchLockState(widget.childUid).listen((state) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _pickTime(bool isStart) async {
    final initial = isStart
        ? TimeOfDay(
            hour: _schedule.startHour,
            minute: _schedule.startMinute)
        : TimeOfDay(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Locking restricts the device to calls and emergency '
                    'functions only. The child can always call emergency services.',
                    style: GoogleFonts.inter(
